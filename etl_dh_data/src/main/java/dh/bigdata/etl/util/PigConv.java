...
public class PigConv {
	...
	public static String getItemcode(DHEvent event) throws Exception {
		String itemcode = null;
		String pattern = "[0-9]*{7,12}";
		if ((event.getTags().get("loc") != null)
				&& Pattern.matches(pattern, event.getTags().get("loc").toString())) {
			itemcode = event.getTags().get("loc").toString();
		} else {
			if (event.getTags().get("itemcode") != null && !"".equals(event.getTags().get("itemcode"))) {
				itemcode = event.getTags().get("itemcode").toString();
			} else {
				itemcode = "";
			}
		}
	...
	public static String getByUuid(DHEvent event) throws Exception{
		String Uuid = null;
		String type = event.getTags().get("type").toString();
		String url = event.getTags().get("url").toString();
		String uuid = event.getTags().get("uuid").toString();
	...
	public static String getSchkw(DHEvent event) throws Exception {
		String schkw = null;
		if ((event.getTags().get("schkw") == null) || event.getTags().get("schkw").toString().length() !=1) {
			schkw = "";
		} return schkw;
		}
