...
public class InsertOdsLogAppEvents implements Function2<JavaRDD<Row>, Time, Void> {
    ...
    public Void call(JavaRDD<Row> rdd, Time time) throws Exception {
        if (!rdd.isEmpty()) {
            ...
            structFields.add(DataTypes.createStructField("currevent", DataTypes.StringType, true));
            structFields.add(DataTypes.createStructField("isfirst", DataTypes.StringType, true));
